void drawStage1Text() {
 textAlign(LEFT);
  text("Progress: " + (100 - (rIndex + 1.0) / VERTEX_COUNT * 100) + "%", 15, 20);
  text("Vertex count: " + (rIndex + 1), 15, 40);
  text("Edge count: " + (edgeCount), 15, 60);
  text("Rotate: " + (rot && allow3DRotate ? "ON" : "OFF"), 15, 80);
  text("Processing: " + (isProcessing ? "ON" : "OFF"), 15, 100);
  text("Edge: " + (showEdges ? "ON": "OFF"), 15, 120);


  text("Highlight Min/Max Nodes: " + (highlightMinMaxDegreeNodes && maximum >= 0 ? "ON" : "OFF"), 15, 140);
  if (highlightMinMaxDegreeNodes && maximum >= 0) {
    fill(0, 255, 0);
    text("Minimum degree: " + (minimum), 15, 160);
    text("Amount: " + (minAmount), 15, 180);
    fill(255, 0, 255);
    text("Maximum degree: " + (maximum), 15, 200);
    text("Amount: " + (maxAmount), 15, 220);
    fill(0, 0, 255);
    text("Highlight edge: " + (isHighlightEdges ? "ON" : "OFF"), 15, 240);
  } 
}
